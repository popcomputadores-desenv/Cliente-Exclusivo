/* SENJU THEME for K-Restaurant Mobile App 2.2*/
...
/** Atualização Master Hub (Correção de Tradução) **/
		 $("#home_search").attr("placeholder", t("What are you looking for?"));  
/** Fim da atualização **/
   	     break;
...
/** Fim da atualização **/
        ...
        break;
   	  ...
   	  case "login":   	      	 
   	   ...
   	   if(!tabbar_loaded){
            $(".stic_uppercase").attr("onclick", "initSignup('1')" );                
   	   } 
...
/** Fim da atualização **/
	 ...
	 placeholder(".contact_phone", 'Contact number'); 
   	  ...
   	  case "order_details":
          $('.order_id').html( t("Order ID#")+" "+page.data.order_id );   	                    
    	 ...
    	 placeholder(".search-input", t("Enter Booking ID or Restaurant Name"));  	     
   	     infinite_page=0;
   	 ...
   	 case "orders":
   	   ...
   	   placeholder(".search-input",'Enter Order ID or Restaurant Name');
   	 break;
   	 ...
   	 case "addressbook_location":
   	   ...
   	   placeholder(".street",'Street');
   	   placeholder(".state_name",'State/Region');
   	   placeholder(".zipcode",'Zip Code');
   	   placeholder(".city_name",'City');
   	   placeholder(".area_name",'Distric/Area/neighborhood');
   	 ...
   	 case "location_state":
   	   ...
   	   placeholder(".search-input",'State');
   	 break;
   	 ...
   	 case "location_area":
   	   ...
   	   placeholder(".search-input",'District/Area');
   	   infinite_page=0;   	      	      	   
   	 ...
   	 case "contact_us":   	   
   	   ...
   	   placeholder(".contact_phone",'Phone');
...
document.addEventListener('postpop', function(event) {
	...
	switch(current_page){
		...
		case "receipt":
		  runOrderHistory(false);
   	      placeholder(".search-input",'Enter Order ID or Transaction Type');
		break;
...
/** Atualização Master Hub (Número e Bairro no Endereço) **/
                     ...
                     $(".area_name").val(data.details.area_name);
/** Fim da atualização **/
			        	$(".city_name").val(  data.details.city );
			        	$(".state_name").val(  data.details.state );
			        	$(".city_name_raw").val(  data.details.city );
			        	$(".state_raw").val(  data.details.state );
	        	...
	        	case "geoCodeMapa":
					...
					buscar_ids_por_Bairro(data.details.city, data.details.area_name);
	        	break;
...
/*Atualização Master Hub (Preenchimento dos dados no Catálogo de Endereços adquiridos pelo CEP)*/
				case "IdsDoCEP":
					...
					$(".area_name_raw").val(data.details.bairro);
					$(".city_name_raw").val(data.details.cidade);
					$(".state_raw").val(data.details.estado);
					$(".location_name").val('');
					...
					$(".state_name").val(data.details.estado);
					$(".city_name").val(data.details.cidade);
		...
		showAlert( t("Preencha o numero da sua residencia ou de onde voce esta!") );		
				...
				case "IdsDoBairro":
					if(!empty(data.details)){
					$(".area_name_raw").val(data.details.area_name);
					$(".city_name_raw").val(data.details.city);
					$(".state_raw").val(data.details.state);
					$(".location_name").val('');
					$(".delivery_instruction").val('');
					...
					$(".city_name").val(data.details.city);
					$(".area_name").val(data.details.area_name);
		showAlert( t("Confira o endereco se esta correto, pois foi preenchido automaticamente pela localizacao do GPS!") );		
					}
					break;
...
/*Atualização Master Hub (Preenchimento dos dados no Catálogo de Endereços adquiridos pelo CEP)*/
				case "IdsDoCEP":
					...
					showAlert( t("Desculpa, ainda nao entregamos no bairro: ")+"<br>"+ data.get.nome_bairro +"<br>"+t("Se este nao for seu bairro selecione seu endereco manualmente!"));
					break;
...
var setFieldValueBairro = function (class_name, value_area, label_area, value_city, label_city, value_state, label_state) {
   ...
   switch (class_name) {
      ...
      case "area_list":
		   ...
		   $(".area_name_raw").val( label_area );
		   $(".city_id").val( value_city );
		   $(".city_name").val( label_city );
		   $(".city_name_raw").val( label_city );
		   $(".state_id").val( value_state );
		   $(".state_name").val( label_state );
		   $(".state_raw").val( label_state );
...
geoCode = function(){
	...
	ajaxCall("geoCode","lat=" + $(".lat").val() + "&lng=" + $(".lng").val() );
};

geoCodeMapa = function(){
	//ajaxCall("geoCode","lat=" + $(".selected_lat").val() + "&lng=" + $(".selected_lng").val() );
	ajaxCall("geoCodeMapa","lat=" + $(".lat").val() + "&lng=" + $(".lng").val() );
};
	...
	if(settings = AppSettings()){
	   if(settings.singleapp_enabled_banner==1){
         ...
         html+='<div class="stic-title">';
            html+='<ons-icon icon="magic"></ons-icon>';
            html+='<div class="home-title">';
               html+='<span class="main trn">Limited time offers</span>';
               html+='<br>';
               html+='<span class="min trn">Enjoy it before it\'s over</span>';
            html+='</div>';
         html+='</div>';

         html+='<div class="stic-carousel-margin">';
	       html+='<ons-carousel swipeable auto-scroll auto-scroll-ratio="0.1" overscrollable id="k_carousel" class="stic-carousel">';
	   	
	   	   $.each( settings.singleapp_banner  , function( key, val ) {
	   	   	html+='<ons-carousel-item class="stic-carousel-item">';
   ...
   removeStorage("profile_avatar");
	...
	if(isLogin()){
		  ...
		  ajax_profile.done(function( data ) {				 	
			 dump(data);					 
			 if(data.code==1){
             // var stic_username = data.details.data.first_name;
...
setAreaListVal = function(area_id,city_name_raw,area_name_raw){
	popPage();
	...
	setTimeout(function(){ 
		...
		buscar_ids_Bairro(city_name_raw,  area_name_raw);
    }, 100);	
};
